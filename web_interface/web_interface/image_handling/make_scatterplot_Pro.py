...
fh = open(('./tmp/%s-%s-%s-%s-map.html' % (mygene1, mygene2,expression,genome)), 'w')
if (bool(myexp1) == True and bool(myexp2) == True):
    ...
    ax.set_xlabel('%s Abundance (NSAF)' % mygene1)
    ax.set_ylabel('%s Abundance (NSAF)' % mygene2)
    ax2.set_xlabel('%s Abundance (NSAF)' % mygene1)
    ax2.set_ylabel('%s Abundance (NSAF)' % mygene2)
    ...
    fig.savefig(('./tmp/%s-%s-%s-%s.svg' % (mygene1, mygene2,expression,genome)), dpi=300)
    fig.savefig(('./tmp/%s-%s-%s-%s.png' % (mygene1, mygene2,expression,genome)), dpi=300)
    fig2.savefig(('./tmp/%s-%s-%s-%s-front.png' % (mygene1, mygene2,expression,genome)), dpi=80)
    # changed here
    ...
    fh.write(
        '<a href="./tmp/%s-%s-%s-%s.png"><IMG SRC="./tmp/%s-%s-%s-%s-front.png" ismap usemap="#points" WIDTH="%d" HEIGHT="%d"></a><MAP name="points">' % (
        mygene1, mygene2,expression,genome, mygene1, mygene2,expression,genome, image_width, image_height))
    ...
    fh.write("""<b>Abundance</b> <SPAN id='myx'></SPAN> NSAF<br>
	""")
    fh.write("<b>%s:</b> " % mygene2)
    fh.write("""<b>Abundance</b> <SPAN id='myy'></SPAN> NSAF
	""")
    fh.close()
else:

    fh.write("<p><b>Abundance data not available for given genes.</b></p>")
