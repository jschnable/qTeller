...
if not 'all' in mychr and mystop != 0:
    c.execute("select * from gene_table where chromosome=? and start>? and stop <?",(mychr,mystart,mystop))
elif not 'all' in mychr and mystop == 0:
    c.execute("select * from gene_table where chromosome=? and start>?", (mychr, mystart))
else:
...
for row in c:
    ...
    p_dict[myname].append("<a href='http://yourwebsite/Rna_Protein_bar_chart.php?name=%s&gene=Submit for mRNA Abundance!&info=%s'>http://yourwebsite/Rna_Protein_bar_chart.php?name=%s&gene=Submit for mRNA Abundance!&info=%s</a>" % (myname,all_info,myname,all_info))
